            ...
            <div className='space-y-3'>
                ...
                <div className='flex gap-2'>
                    {suggestedKeywords.map((word) => (
                        <div key={word} className='p-3 h-5 border-1 border-white flex items-center justify-center rounded-md text-white hover:bg-background hover:text-black cursor-pointer transition-all' onClick={() => navigate(`/search/${word}`)}>
                            <p className=''>{word}</p>
                        </div>
                    ))}
                </div>
